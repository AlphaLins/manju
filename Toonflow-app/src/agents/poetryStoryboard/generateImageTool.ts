...
async function filterRelevantAssets(prompts: string[], allResources: ResourceItem[], availableImages: ImageInfo[]): Promise<ImageInfo[]> {
  ...
  const apiConfig = await u.getPromptAi("poetryStoryboardAgent");
...
export default async (cells: { prompt: string }[], scriptId: number, projectId: number) => {
  const scriptData = await u.db("t_poetry_script").where({ id: scriptId, projectId }).first();
  const projectInfo = await u.db("t_project").where({ id: projectId }).first();

  const row = await u.db("t_poetry_outline").where({ id: scriptData?.outlineId!, projectId }).first();
  ...
  const imagesRaw = await u.db("t_poetry_assets").whereIn("name", resourceNames).andWhere({ projectId }).select("name", "type", "filePath");
  ...
  const apiConfig = await u.getPromptAi("poetryStoryboardImage");
