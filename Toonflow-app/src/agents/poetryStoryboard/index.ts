...
export default class Storyboard {
  ...
  getScript = tool({
    ...
    execute: async () => {
      ...
      const script = await u.db("t_poetry_script").where({ id: this.scriptId, projectId: this.projectId }).first();
  ...
  getAssets = tool({
    ...
    execute: async () => {
      ...
      const scriptData = await u.db("t_poetry_script").where({ id: this.scriptId, projectId: this.projectId }).first();
      const row = await u.db("t_poetry_outline").where({ id: scriptData?.outlineId!, projectId: this.projectId }).first();
...
3. 禁止捏造资产列表中不存在的角色、场景、道具`;
  ...
  private async buildEnvironmentContext(): Promise<string> {
    ...
    const row = await u.db("t_poetry_outline").where({ id: this.scriptId, projectId: this.projectId }).first();
    const outline: any | null = row?.data ? JSON.parse(row.data) : null;
...
${task}
  ...
  private async invokeSubAgent(agentType: AgentType, task: string): Promise<string> {
    ...
    const promptsList = await u.db("t_prompts").where("code", "in", ["poetryStoryboard-segment", "poetryStoryboard-shot"]);
    const promptConfig = await u.getPromptAi("poetryStoryboardAgent");
    ...
    const segmentAgent = getAiPromptConfig("poetryStoryboard-segment");
    const shotAgent = getAiPromptConfig("poetryStoryboard-shot");
  ...
  async call(msg: string): Promise<string> {
    ...
    const prompts = await u.db("t_prompts").where("code", "poetryStoryboard-main").first();
    const promptConfig = await u.getPromptAi("poetryStoryboardAgent");
