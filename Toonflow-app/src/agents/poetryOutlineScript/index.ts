...
export default class OutlineScript {
  ...
  novelChapters: DB["t_poetry_novel"][] = [];
  ...
  setNovel(chapters: DB["t_poetry_novel"][]) {
  ...
  private async getNovelInfo(asString = false): Promise<any> {
    ...
    if (asString) {
      const fields = [
        ...
        `作品简介: ${info.intro}`,
        `作品类型: ${info.type}`,
        `目标视频风格: ${info.artStyle}`,
        `视频画幅: ${info.videoRatio}`,
      ];
      return fields.join("\n");
    }
    return info;
  ...
  private async findStoryline() {
    return u.db("t_poetry_storyline").where({ projectId: this.projectId }).first();
  }

  private async upsertStorylineContent(content: string) {
    console.log("[调试-后端] upsertStorylineContent 开始, projectId:", this.projectId);
    const existing = await this.findStoryline();
    console.log("[调试-后端] 查询现有故事线:", existing ? "存在" : "不存在");
    if (existing) {
      console.log("[调试-后端] 执行更新操作");
      await u.db("t_poetry_storyline").where({ projectId: this.projectId }).update({ content });
    } else {
      console.log("[调试-后端] 执行插入操作");
      await u.db("t_poetry_storyline").insert({ projectId: this.projectId, content });
    }
    console.log("[调试-后端] 数据库操作完成, 准备发送 refresh 事件");
    this.refresh("storyline");
    console.log("[调试-后端] refresh 事件已发送");
  }

  private async deleteStorylineContent() {
    const deleted = await u.db("t_poetry_storyline").where({ projectId: this.projectId }).del();
    this.refresh("storyline");
    return deleted;
  }
  ...
  private async findOutlines() {
    return u.db("t_poetry_outline").where({ projectId: this.projectId }).orderBy("episode", "asc");
  }

  private async findOutlineById(id: number) {
    return u.db("t_poetry_outline").where({ id, projectId: this.projectId }).first();
  }

  private async getMaxEpisode(): Promise<number> {
    const result: any = await u.db("t_poetry_outline").where({ projectId: this.projectId }).max("episode as max").first();
    return result?.max ?? 0;
  }

  private async clearOutlinesAndScripts() {
    const outlines = await u.db("t_poetry_outline").select("id").where({ projectId: this.projectId });
    if (outlines.length === 0) return 0;

    const outlineIds = outlines.map((o) => o.id);
    await u.db("t_poetry_script").whereIn("outlineId", outlineIds).del();
    await u.db("t_poetry_outline").where({ projectId: this.projectId }).del();
  ...
  private async insertOutlines(episodes: EpisodeData[], startEpisode: number) {
    ...
    await u.db("t_poetry_outline").insert(insertList);
  ...
  private async createEmptyScripts(outlineIds: Array<{ id: number; data: string }>) {
    ...
    if (scripts.length > 0) {
      await u.db("t_poetry_script").insert(scripts);
    }
...
道具(按出场顺序): ${ep.props?.map((p: AssetItem) => `${p.name}(${p.description})`).join("; ") || "无"}`;
  ...
  private async findAssetByTypeAndName(type: AssetType, name: string) {
    return u.db("t_poetry_assets").where({ projectId: this.projectId, type, name }).first();
  }
  ...
  private async upsertAsset(type: AssetType, item: AssetItem): Promise<"inserted" | "updated" | "skipped"> {
    ...
    if (!existing) {
      await u.db("t_poetry_assets").insert({
        projectId: this.projectId,
        type,
        name: item.name,
    ...
    if (existing.intro !== item.description) {
      await u.db("t_poetry_assets").where({ id: existing.id }).update({
        intro: item.description,
        prompt: item.description,
      });
      return "updated";
  ...
  private async generateAssetsFromOutlines() {
    const outlines = await u.db("t_poetry_outline").select("data").where({ projectId: this.projectId });
  ...
  saveStoryline = tool({
    ...
    execute: async ({ content }) => {
      this.log("保存故事线");
      console.log("[调试-后端] saveStoryline 被调用, projectId:", this.projectId);
      console.log("[调试-后端] saveStoryline content 长度:", content?.length);
      console.log("[调试-后端] saveStoryline content 前100字符:", content?.substring(0, 100));
      await this.upsertStorylineContent(content);
      console.log("[调试-后端] saveStoryline 保存完成");
      return "故事线保存成功";
    },
  });
  ...
  private getChapterContext(): string {
    ...
    return this.novelChapters.map((c) => `章节号:${c.chapterIndex}，分卷:${c.reel || "默认"}，章节名:${c.chapter || "未命名章节"}`).join("\n");
  }
  ...
  private async buildEnvironmentContext(): Promise<string> {
    const [novelInfo, storyline, outlineCount] = await Promise.all([
      ...
      this.findStoryline(),
      u.db("t_poetry_outline").where({ projectId: this.projectId }).count("id as count").first() as any,
    ]);
...
${task}
  ...
  private async invokeSubAgent(agentType: AgentType, task: string): Promise<string> {
    ...
    const promptsList = await u.db("t_prompts").where("code", "in", ["poetryOutlineScript-a1", "poetryOutlineScript-a2", "poetryOutlineScript-director"]);
    const promptConfig = await u.getPromptAi("poetryOutlineScriptAgent");
    ...
    const a1Prompt = getAiPromptConfig("poetryOutlineScript-a1");
    const a2Prompt = getAiPromptConfig("poetryOutlineScript-a2");
    const directorPrompt = getAiPromptConfig("poetryOutlineScript-director");
  ...
  async call(msg: string): Promise<string> {
    ...
    const prompts = await u.db("t_prompts").where("code", "poetryOutlineScript-main").first();
    const promptConfig = await u.getPromptAi("poetryOutlineScriptAgent");

    console.log("[调试-大纲助手] projectId=", this.projectId, "章节数=", this.novelChapters.length, "promptCode=poetryOutlineScript-main");
