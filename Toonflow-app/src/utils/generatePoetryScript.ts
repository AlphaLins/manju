 ...
 */
export async function generatePoetryScript(episode: Episode, novelData: string): Promise<string> {
...
${novelData}`;
  ...
  const prompts = await u.db("t_prompts").where("code", "poetryGenerateScript").first();
  const promptConfig = await u.getPromptAi("poetryGenerateScript");
  const mainPrompts = prompts?.customValue || prompts?.defaultValue || "不论用户说什么，请直接输出AI配置异常";
  console.log("poetry prompt len:", mainPrompts?.length || 0);
