...
export default router.post(
  ...
  async (req, res) => {
    ...
    const existing = await u.db(tableName).where({ projectId }).first();
    if (existing) {
      await u.db(tableName).where({ projectId }).update({ content });
    } else {
      await u.db(tableName).insert({ projectId: projectId, content: content });
    }
