...
import PoetryOutlineScript from "@/agents/poetryOutlineScript";
const router = express.Router();
expressWs(router as unknown as Application);

router.ws("/", async (ws, req) => {
  let agent: OutlineScript | PoetryOutlineScript;
  let agentStyle = "normal";
  ...
  ws.send(JSON.stringify({ type: "init", data: { projectId, agentStyle } }));
  ...
  ws.on("message", async function (rawData: string) {
    ...
    const novelData = await u
      .db(agentStyle === "poetry" ? "t_poetry_novel" : "t_novel")
      .where({ projectId: Number(projectId) })
    ...
    try {
      switch (data?.type) {
        ...
        case "cleanHistory":
          agent.history = [];
          await u
            .db(agentStyle === "poetry" ? "t_poetry_chatHistory" : "t_chatHistory")
            .where({ projectId: Number(projectId) })
            .del();
  ...
  async function saveHistory() {
    ...
    const tableName = agentStyle === "poetry" ? "t_poetry_chatHistory" : "t_chatHistory";
    //保存对话记录
    const existing = await u
      .db(tableName)
