...
export default router.post(
  ...
  async (req, res) => {
    ...
    const assets = await u
      ...
      .where("type", "分镜")
      .select("id", "name", "intro", "prompt", "filePath", "duration", "poetryVideoPrompt", "scriptId", "type", "segmentId", "shotIndex").orderBy("segmentId", "asc").orderBy("shotIndex", "asc");

    const assetsIds = assets.map((item: any) => item.id);

    const generateImg = await u.db("t_poetry_image").whereIn("assetsId", assetsIds).where("type", "分镜").select("assetsId", "filePath");
