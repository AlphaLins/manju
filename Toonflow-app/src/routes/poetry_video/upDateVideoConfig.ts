...
export default router.post(
  ...
  async (req, res) => {
    ...
    // 检查配置是否存在
    const existingConfig = await u.db("t_poetry_videoConfig").where({ id }).first();
    if (!existingConfig) {
    ...
    // 更新数据
    await u.db("t_poetry_videoConfig").where({ id }).update(updateData);

    // 获取更新后的数据
    const updatedConfig = await u.db("t_poetry_videoConfig").where({ id }).first();
    if (updatedConfig) {
      res.status(200).send(
        success({
          message: "更新视频配置成功",
