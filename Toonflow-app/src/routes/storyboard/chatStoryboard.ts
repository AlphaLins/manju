...
import PoetryStoryboard from "@/agents/poetryStoryboard";
const router = express.Router();
expressWs(router as unknown as Application);

router.ws("/", async (ws, req) => {
  let agent: Storyboard | PoetryStoryboard;
  ...
  if (project?.agentStyle === "poetry") {
    agentStyle = "poetry";
    agent = new PoetryStoryboard(Number(projectId), Number(scriptId));
  } else {
  ...
  ws.send(JSON.stringify({ type: "init", data: { projectId, scriptId, agentStyle } }));
  ...
  async function saveHistory() {
    ...
    const historyType = agentStyle === "poetry" ? "poetryStoryboardAgent" : "storyboardAgent";
    //保存对话记录
    const existing = await u
      .db(tableName)
      .where({ projectId: Number(projectId), type: historyType })
      .first();
    if (existing) {
      await u
        .db(tableName)
