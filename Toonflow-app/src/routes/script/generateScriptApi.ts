...
export default router.post(
  ...
  async (req, res) => {
    ...
    const outline = await u.db("t_outline").where("id", outlineId).first();
    const projectId = outline?.projectId;
    if (!projectId) return res.status(500).send(success({ message: "大纲为空" }));
    ...
    const novelTable = project?.agentStyle === "poetry" ? "t_poetry_novel" : "t_novel";
    ...
    try {
      ...
      await u.db(scriptTable).where("id", scriptId).update({
        content: data,
      });
