...
export default router.post(
  ...
  async (req, res) => {
    ...
    if (base64) {
      ...
      // 插入图片表
      await u.db("t_poetry_image").insert({
        assetsId: id,
        filePath: savePath,
        type: "image/png",
      });
      ...
      // 检查图片表里是否有这条图片
      // const selectedImage = await u.db("t_poetry_image").where("filePath", savePath).first();
      // if (!selectedImage) {
    ...
    // 查旧资产图片
    const oldAsset = await u.db("t_poetry_assets").where("id", id).select("filePath", "type").first();

    // 保存新旧图片差异和插临时表逻辑
    if (imageUrl && ((oldAsset?.filePath && oldAsset.filePath !== imageUrl) || (!oldAsset?.filePath && imageUrl))) {
      // 新图片保存，移除 t_image 表
      await u.db("t_poetry_image").where("filePath", imageUrl).delete();

      // 原图片如果存在、且不在 t_image 表，插入临时表
      if (oldAsset?.filePath) {
        const oldInTemp = await u.db("t_poetry_image").where("filePath", oldAsset.filePath).first();
        if (!oldInTemp) {
          await u.db("t_poetry_image").insert({
            assetsId: id,
      ...
      // 更新资产表图片为新图片
      await u.db("t_poetry_assets").where("id", id).update({ filePath: imageUrl });
    }

    // 更新提示信息
    if (prompt !== undefined && prompt !== null && prompt !== "") {
      await u.db("t_poetry_assets").where("id", id).update({ prompt });
    }
