...
export default router.post(
  ...
  async (req, res) => {
    ...
    const allOutlineDataList: { data: string }[] = await u.db("t_poetry_outline").where("projectId", projectId).select("data");
    ...
    const promptsList = await u.db("t_prompts").where("code", "in", ["role-polish", "scene-polish", "poetryStoryboard-polish", "tool-polish"]);
    const apiConfigData = await u.getPromptAi("poetryAssetsPrompt");
    ...
    const storyboard = getPromptValue("poetryStoryboard-polish");
    let systemPrompt = "";
    let userPrompt = "";
    if (type == "role") {
      const data = findItemByName(result, name, "characters");
      const chapterRange = Array.isArray(data?.chapterRange) ? data.chapterRange : data?.chapterRange ? [data.chapterRange] : [];
      const novelData = chapterRange.length > 0 ? (await u.db("t_poetry_novel").whereIn("chapterIndex", chapterRange).select("*")) as NovelChapter[] : [];
