...
export default router.post(
  ...
  validateFields({
    id: z.number(),
    type: z.enum(["role", "scene", "props", "poetryStoryboard"]),
    projectId: z.number(),
    name: z.string(),
  ...
  async (req, res) => {
    ...
    const promptsList = await u
      .db("t_prompts")
      .where("code", "in", ["role-generateImage", "scene-generateImage", "poetryStoryboard-generateImage", "tool-generateImage"]);
    ...
    const storyboard = getPromptValue("poetryStoryboard-generateImage");
    ...
    **分镜设定：**
    ...
    const [imageId] = await u.db("t_poetry_image").insert({
      state: "生成中",
      assetsId: id,
    });
    const apiConfig = await u.getPromptAi("poetryAssetsImage");
    ...
    try {
      ...
      const imageData = await u.db("t_poetry_image").where("id", imageId).select("*").first();
      if (imageData) {
        await u.db("t_poetry_image").where("id", imageId).update({
          state: "生成成功",
          filePath: imagePath,
          type: insertType,
        });
        ...
        // const state = await u.db("t_poetry_assets").where("id", id).select("state").first();
