...
// 获取小说列表
const getNovel = () => {
  axios.post("/poetry_novel/getNovel", { projectId: projectId.value }).then((res) => {
...
const handleAddChapters = (chapters: ChapterItem[]) => {
  axios.post("/poetry_novel/addNovel", { projectId: projectId.value, data: chapters }).then(() => {
    getNovel();
...
// 更新
const handleUpdate = () => {
  axios.post("/poetry_novel/updateNovel", formData.value).then(() => {
    getNovel();
...
const handleDelete = (row: OriginalText) => {
  axios.post("/poetry_novel/delNovel", { id: row.id }).then(() => {
    getNovel();
  });
};
