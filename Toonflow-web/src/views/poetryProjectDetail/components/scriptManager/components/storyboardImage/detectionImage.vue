...
async function handleBatchSuperScore() {
  ...
  try {
    ...
    const res = await axios.post("/poetry_storyboard/batchSuperScoreImage", payload);
...
async function handleBatchGeneratePrompts() {
  ...
  const requests = rowsToProcess.map((row) =>
    axios
      .post("/poetry_storyboard/generateVideoPrompt", {
...
function handleOk() {
  ...
  axios
    .post("/poetry_storyboard/keepStoryboard", {
