                ...
                <div class="section-header">
                  ...
                  <div class="f ac">
                    <div class="exportScript">
                      <a-button @click="handleGenerate" :disabled="generating" size="small" class="batch-btn">
...
async function handleGenerate() {
  ...
  try {
    await axios.post("/poetry_script/generateScriptApi", { outlineId, scriptId: id });
...
async function saveScript() {
  ...
  try {
    await axios.post("/poetry_script/generateScriptSave", {
...
async function handleBatchSave(list: Element[]) {
  const tasks = list.map((item) =>
    Promise.all([
      axios.post("/poetry_assets/updateAssets", {
        id: item.id,
        name: item.name,
        intro: item.intro,
        type: item.type,
      ...
      axios.post("/poetry_assets/saveAssets", {
        id: item.id,
  ...
  await Promise.all(types.map((type) => axios.post("/poetry_assets/getAssets", { projectId: projectId.value, type })));
  await fetchScripts();
}
