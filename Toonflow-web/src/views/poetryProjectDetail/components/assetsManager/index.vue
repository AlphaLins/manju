...
async function fetchAssets(type: string) {
  const res = await axios.post("/poetry_assets/getAssets", { projectId: projectId.value, type: typeToParam(type) });
  projectElements.value = res.data;
}

async function fetchStoryboardList() {
  const res = await axios.post("/poetry_assets/getStoryboard", { projectId: projectId.value });
  return res.data;
}

async function fetchStoryboardDetail(scriptId: number) {
  const res = await axios.post("/poetry_storyboard/getStoryboard", { projectId: projectId.value, scriptId });
  projectElements.value = res.data;
}
...
async function handleBatchGeneratePrompt() {
  ...
  await processBatch(selectModal.value, async (item) => {
    try {
      const res = await axios.post("/poetry_assets/polishPrompt", {
        projectId: projectId.value,
        assetsId: item.id,
...
async function handleBatchGeneration() {
  ...
  await processBatch(selectModal.value, async (item) => {
    try {
      const res = await axios.post("/poetry_assets/generateAssets", {
        type: typeMap[item.type] ?? "props",
        projectId: projectId.value,
        name: item.name,
        prompt: item.prompt,
