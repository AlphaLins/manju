...
const fetchAssets = async (type: string) => {
  const { data } = await axios.post("/poetry_assets/getAssets", { projectId: projectId.value, type: typeToParam(type) });
  projectElements.value = data;
};

const fetchScriptList = async () => {
  const { data } = await axios.post("/poetry_assets/getStoryboard", { projectId: projectId.value });
  return data;
};

const fetchStoryboard = async (scriptId: number) => {
  const { data } = await axios.post("/poetry_storyboard/getStoryboard", { projectId: projectId.value, scriptId });
  projectElements.value = data;
};
...
const handleBatchSave = async (list: ElementData[]) => {
  ...
  await batchProcess(list, otherSetting.value.assetsBatchGenereateSize || 5, async (item) => {
    await axios.post("/poetry_assets/updateAssets", {
      id: item.id,
      name: item.name,
      intro: item.intro ?? "",
      type: item.type,
    ...
    await axios.post("/poetry_assets/saveAssets", {
      id: item.id,
      base64: "",
...
const handleBatchGeneration = async () => {
  ...
  try {
    await batchProcess(selectModal.value, 5, async (item) => {
      const { data } = await axios.post("/poetry_assets/generateAssets", {
...
const handleBatchGeneratePrompt = async () => {
  ...
  try {
    await batchProcess(selectModal.value, 5, async (item) => {
      const { data } = await axios.post("/poetry_assets/polishPrompt", {
        projectId: projectId.value,
        assetsId: item.id,
        type: TYPE_MAP[item.type] ?? "props",
        name: item.name,
...
const deleteFrom = (row: ElementData) => {
  Modal.confirm({
    ...
    cancelText: "取消",
    onOk: async () => {
      await axios.post("/poetry_assets/delAssets", { id: row.id });
