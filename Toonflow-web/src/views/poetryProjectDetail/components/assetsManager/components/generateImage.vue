...
async function fetchImages(id: number) {
  const _id = id;
  const { data } = await axios.post("/poetry_assets/getImage", { assetsId: id });
...
async function delImage(item: ImageState) {
  Modal.confirm({
    ...
    onOk: async () => {
      if (item.id) {
        await axios.post("/poetry_assets/delAssetsImage", {
          imageId: item.id,
        });
      } else {
        await axios.post("/poetry_assets/delAssetsImage", {
...
async function startGenerate() {
  ...
  try {
    const _promise = axios.post("/poetry_assets/generateAssets", {
...
async function handleSave() {
  ...
  try {
    await axios.post("/poetry_assets/saveAssets", params);
