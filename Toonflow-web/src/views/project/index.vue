    ...
    <div v-else class="list">
      <div v-for="project in projects" :key="project.id" class="card" @click="openProject(project)">
        <div class="content">
          ...
          <div class="time">
            <span>风格：{{ project.agentStyle === "poetry" ? "古诗特化" : "普通" }}</span>
...
const openProject = async (project: any) => {
  try {
    await store().setProjectById(project.id);
    const targetPath = project.agentStyle === "poetry" ? `/poetry?id=${project.id}` : `/projectDetail?id=${project.id}`;
    await router.push(targetPath);
  } catch (e) {
    console.error("Router/Store push error:", e);
    message.error("无法打开该项目，请查看控制台: " + String(e));
  }
};
